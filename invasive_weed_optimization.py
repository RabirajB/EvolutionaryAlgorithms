...
def iwo_algorithm(Tx,Ty,weedlist,itermax,pmax,Smax,Smin,sigmafinal,sigmainit,n): # n = modulation index
   ...
   for i in range(1, itermax+1):
        ...
        fmax, _ = get_max_fitness(weedlist)
        fmin, _ = get_min_fitness(weedlist)
        for weed in weedlist:
...
def iwo_test(Tx,Ty, lenTx, lenTy):
    #lenTx = Tx.size
    #lenTy = lenTx
    weedlist=[]
    for i in range(10):
    ...
    wlist = iwo_algorithm(Tx, Ty, weedlist, 15, 150, 10, 1, 0.01, 1, 3)
    fmax, bestweed = get_max_fitness(wlist)
    '''for weed in wlist:
    ...
    Wxnew = np.sum(bestweed.Wx)//Wx.size
    #print(Wxnew)
    Wynew = np.sum(bestweed.Wy)//Wy.size
    #print(Wynew)
...
if __name__ == '__main__':
    ...
    Tx, Ty = np.load("terminal_point_{}_{}.npy".format(str(500), str(10)))
    print(Tx)
    print(Ty)
    lenTx = Tx.size
    lenTy = lenTx
